...
double look_ahead_dist, goalRadius, speed, flag;
double myrobot, STOP_Dist;
double prev_steer = 0;
double prev_speed = 0;
int STOP_Dist = -1;
int flag = 0;
...
pose poseUpdate(radl_in_t * in)
{
    // orientation data
    quaternion q;
    pose e;
    q.x = in->pose->orientation->x;
    q.y = in->pose->orientation->y;
    q.z = in->pose->orientation->z;
    q.w = in->pose->orientation->w;
    ...
    if (fabs(sinp) >= 1)
	  ...
	  // pose data
	  e.x = in->pose->position->x;
	  e.y = in->pose->position->y;
...
double calculate_desired_steer(double carrot_x, double carrot_y, double curr_x, double curr_y, double curr_yaw){
  ...
  cout << currt_x << " " << curr_y << " " << carrot_x << " " << carrot_y << endl;
...
void control(const radl_in_t * in, radl_out_t * out){
    ...
    if ( 1 == flag ){
      ...
      vector<double>::iterator min_it = *min_element(begin(startingPoints), end(startingPoints));
      int startIndex = (int)(min_it - it);
      cout << "startIndex = " << startIndex << endl;
      
      // Find carrot point on path
      vector<double> possible_l;
      for( i=0; i < planner_coord; i ++ ){
        possible_l.push_back(0.0);
      }
      
      for ( i = startIndex; i < planner_coord; i ++ ){
        double d = dist(path_x[i], myrobot.x, path_y[i], myrobot.y);
        double difference = abs(d - look_ahead_dist);
        possible_l[i] = difference;
      }
      
      double l_carrot = *min_element((begin(possible_l)+startIndex), end(possible_l));
...
void pure_pursuit::step(const radl_in_t * in, const radl_in_flags_t* inflags,
