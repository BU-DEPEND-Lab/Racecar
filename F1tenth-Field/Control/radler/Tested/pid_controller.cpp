...
pose poseUpdate(const radl_in_t * in)
{
    ...
    //cout << "q.x is :::::::: " << in->slam_out_pose->orientation_z << endl;
    ...
    if (fabs(sinp) >= 1)
	  ...
	  cout <<"curr_pose = " <<  in->slam_out_pose->position_x << " " << e.y << endl;
...
double PD_controller(double desired_steer, double diff_error, double kp, double kd){
...
void PidController::control(const radl_in_t * in, radl_out_t * out){
    ...
    //string pids = exec(" pkill -SIGINT -f master_test");
		...
		//exit(0);
	      ...
	      vector<double> startingPoints;
			cout << planner_coord <<" " << path_x.size()<<endl;
        ...
        double kd = 0.09;
	      ...
	      double steerOutput = PD_controller(desired_steer, diff_error, kp, kd);
...
/*
void getPDvalues(int argc, char *argv[]){

    if (argc != 3){
      cout <<"enter kp and kd values" << endl;
    }
    kp = atof(argv[1]);
    kd = atof(argv[2]);
    cout <<" inside getPDvalues" << endl;
...
PidController::PidController (){
  ...
  const char *tmp;
  ifstream myfile ("/home/ubuntu/catkin_ws/src/race/src/path.txt");
...
void PidController::step(const radl_in_t * in, const radl_in_flags_t* inflags,
	...
	//cout << "incoming pose msg y" << in->slam_out_pose->position_y << endl;
