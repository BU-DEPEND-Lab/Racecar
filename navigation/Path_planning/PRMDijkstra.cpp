...
string outpath = homedir + "/Desktop/Akash/Path_planning/Generator/LooseFiles/Paths_PRM/path";
...
void DisplayPath(vector<vector<int>> path, int val){
        for (int i = 0; i < path.size(); i ++){
            ...
            Neighbors(1,path[i][0],path[i][1]);
            for( int i = 0; i<nei.size(); i++){
                m[nei[i][0]][nei[i][1]] = val;
            }
        }
...
//                          Function Definition
//   Name  - get_pt_list
//   Function  - Returns list of valid points for roadmap
//   Arguments  - int num_pt
/////////////////////////////////////////////////////////////////////////////////

vector<vector<int>> get_pt_list( int num_pt){
    vector<vector<int>> validList;
    int xtemp = 0,ytemp = 0, count = 0;
    while(count<num_pt){
        vector<int> temp;
        xtemp = (rand()) % 2048;
        ytemp = (rand()) % 2048;
        if (int(m[xtemp][ytemp]) == 255){
            temp.push_back(xtemp);
            temp.push_back(ytemp);
            validList.push_back(temp);
            count++;
        }
...
//                          Function Definition
//   Name  - get_path_list
//   Function  - Returns adjacency matrix of valid edges for roadmap
//   Arguments  - vector<vector<int>> verteces
/////////////////////////////////////////////////////////////////////////////////

map <vector<int>, vector<vector<int>>> get_path_list( vector<vector<int>> verteces){
    map <vector<int>, vector<vector<int>>> matrix;
    vector<int> temp;
    for (int i = 0; i<verteces.size(); i++){
        for(int j = 0; j<verteces.size(); j++){
            if(CheckValid(verteces[i], verteces[j]) == 1){
                matrix[verteces[i]].push_back(verteces[j]);
            }
        }
    }
    return matrix;
...
void DisplayTree(map <vector<int>, vector<vector<int>>> Tree, int val) {
    map<vector<int>, vector<vector<int>>>::iterator it;

    for ( it = Tree.begin(); it != Tree.end(); it++ ){
        for(int ii = 0; ii<it->second.size();ii++){
           DisplayPath(LineComp({it->first,it->second[ii]}), val);
        }
    }
    Mat dataMatrix1(2048,2048,CV_8UC1, m);
    namedWindow("Display window",CV_WINDOW_NORMAL);
...
void DisplayLine(vector<vector<int>> path , int val) {
        for (int i = 0; i < path.size(); i ++){
            m[path[i][0]][path[i][1]] = val;
            Neighbors(1,path[i][0],path[i][1]);
            for( int i = 0; i<nei.size(); i++){
...
vector<vector<int>> reconstruct_path( map<vector<int>, nodes>cameFrom, vector<int>current ){
    vector<vector<int>> LineFinal;
    vector<int> temp;
    map<vector<int>, nodes>::iterator it;
    LineFinal.push_back(current);
    it = cameFrom.find(current);
    while((it->first != start)) {
        // cout << current[0] << " " << current[1] << " Parent " << cameFrom[current].parent.first << " " << cameFrom[current].parent.second << endl;
        temp.push_back(cameFrom[current].parent.first);
        temp.push_back(cameFrom[current].parent.second);
        current = temp;
        LineFinal.push_back(current);
        it = cameFrom.find(current);
        temp.clear();
...
int main(int argc, char** argv) {
    ...
    srand(time(NULL));
    validList = get_pt_list(pt_drop);
    validList.push_back(start);
    validList.push_back(stop);
     // DisplayPath(validList);
    get_path_list(validList);
    map<vector<int>, nodes>SPC; // Shortest Path Connections
    map <vector<int>, vector<vector<int>>> validPaths;
    int path_trials = 0;
    int redo_trials = 0;
    while((validPaths[stop].size() == 0) || (validPaths[start].size() == 0)){
        validPaths = get_path_list(validList);
        path_trials++;
        if(path_trials >= 10){
            validList = get_pt_list(pt_drop);
            validList.push_back(start);
     ...
     // DisplayTree(validPaths, 10);
    vector<vector<int>> presentSet, nextSet;
    presentSet.push_back(start);
    SPC[start].dist = 0;
    int flag = 0;
    while(presentSet.size()!=0){

        for(int i = 0; i<presentSet.size(); i++){
            for(int j = 0; j<validPaths[presentSet[i]].size(); j++){
                if(SPC[validPaths[presentSet[i]][j]].dist > (LineComp({presentSet[i],validPaths[presentSet[i]][j]}).size() + SPC[presentSet[i]].dist)) {
                    SPC[validPaths[presentSet[i]][j]].dist = LineComp({presentSet[i],validPaths[presentSet[i]][j]}).size() + SPC[presentSet[i]].dist;
                    SPC[validPaths[presentSet[i]][j]].parent.first = presentSet[i][0];
                    SPC[validPaths[presentSet[i]][j]].parent.second = presentSet[i][1];
                    flag = 1;
                }
                if(SPC[validPaths[presentSet[i]][j]].dist == 9999 || flag == 1){
                    nextSet.push_back(validPaths[presentSet[i]][j]);
                    flag = 0;
                }
            }
        }
    ...
    //     cout<< it->first[0] << " " << it->first[1] << " Dist = " << it->second.dist << " Parent " << it->second.parent.first << " " << it->second.parent.second << endl;
    // }
    LineFinal = reconstruct_path(SPC, stop);
    // DisplayLine(LineComp(LineFinal), 10);
